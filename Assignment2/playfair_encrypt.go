...
func encrypt(input string, keyT [5][5]string) string {
  var arr [4]int
  output := "" 
  for i:=0; i < len(input); i+=2 {
    ...
    if(arr[0] == arr[2]) {
      output += keyT[arr[0]][(arr[1] + 1) %5]
      output += keyT[arr[0]][(arr[3] + 1) %5]
    } else if (arr[1] == arr[3]) {
      output += keyT[arr[0] + 1 % 5][arr[1]]
      output += keyT[arr[2] + 1 % 5][arr[1]]
    } else {
...
func encrypt_handler(input string, key string) string {
  var keyT [5][5]string
  ...
  */
  return encrypt(preprocessed_inp, keyT)
}
